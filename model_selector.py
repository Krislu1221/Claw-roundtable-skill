...
class ModelSelector:
    ...
    def _load_available_models(self):
        ...
        # 优先级 4: 从实例的 models.json 读取（最高优先级本地配置）
        try:
    ...
    def _load_from_instance_models_json(self) -> Optional[List[Dict]]:
        """
        从实例的 models.json 文件读取模型配置（最高优先级本地配置）
        
        Returns:
            模型列表，如果失败返回 None
        """
        try:
            # 查找实例的 models.json 文件
            possible_paths = [
                Path.home() / '.enhance-claw' / 'instances' / '虾软' / 'workspace' / 'agents' / 'main' / 'agent' / 'models.json',
                Path.home() / '.enhance-claw' / 'instances' / '虾总' / 'workspace' / 'agents' / 'main' / 'agent' / 'models.json',
                Path.home() / '.enhance-claw' / 'instances' / '虾编' / 'workspace' / 'agents' / 'main' / 'agent' / 'models.json',
            ]
